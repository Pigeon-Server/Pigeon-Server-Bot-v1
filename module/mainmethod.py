...
async def pass_all():
    ...
    if success == 1:
        for row in output_list:
            id = row[0]
            judge1 = cursor.execute("SELECT id,QQ,player_name from wait where pass is NULL and `id` = '{id}'".format(id=id))
            output = cursor.fetchall()
            if judge1 == 1:
                QQ = output[0][1]
                ...
                if statu != -1:
                    ...
                    if statu1 == 0:
                        ...
                        await say_group(config["admin_qun"], "该玩家已拥有白名单")
                    else:
        ...
        await say_group(config["admin_qun"], "没有待审核的白名单申请")
    del output_list


async def pass_one(id):
    db.ping(reconnect=True)
...
# 退回白名单
async def refuse_one(id, reason=None):
    db.ping(reconnect=True)
    Inquire_1 = cursor.execute("SELECT id,QQ,player_name from wait where pass is not True and pass is not False and `id` = '{id}'".format(id=id))
    output = cursor.fetchall()
    if Inquire_1 == 1:
...
# 申请白名单
async def get_whitelist(playername, qq):
    db.ping(reconnect=True)
    ...
    if Inquire_1 != 1 and Inquire_2 != 1:
        log.info("[审核]已收到玩家{player}的白名单请求".format(player=playername))
        cursor.execute("INSERT INTO wait(`QQ`, `player_name`) VALUES ('{QQ}', '{Player_Name}')".format(database=config["database"], QQ=qq, Player_Name=playername))
        statu = await sql_run()
        if statu == -1:
        ...
        if success == 1:
            reason = data[0][0]
            await say_group(config["player_qun"], '您的白名单申请权限已锁定\n'
                                 '原因：白名单申请被管理组拒绝\n'
...
# 取消绑定
async def unbundling(playername, qq):
    db.ping(reconnect=True)
    Inquire_1 = cursor.execute("SELECT * from wait where QQ = '{QQ}' and player_name = '{player}'".format(QQ=qq, player=playername))
    Inquire_2 = cursor.execute("SELECT * from whitelist where QQ = '{QQ}' and player_name = '{player}'".format(QQ=qq, player=playername))
    Inquire_3 = cursor.execute("SELECT * from banlist where player_name = '{player}'".format(player=playername))
    Inquire_4 = cursor.execute("SELECT * from wait where QQ = '{QQ}' and player_name = '{player}' and pass = 0".format(QQ=qq, player=playername))
    print(Inquire_1, Inquire_2, Inquire_3, Inquire_4)
    if Inquire_3 == 1:
        ...
        if Inquire_1 == 1:
            ...
            if statu1 == -1:
                await say_group_warning(config["admin_qun"], "数据库执行错误")
        if Inquire_2 == 1:
            statu2 = await sql_run("DELETE from whitelist where QQ = '{QQ}' and player_name = '{player}'".format(QQ=qq, player=playername))
            if statu2 == -1:
                await say_group_warning(config["admin_qun"], "数据库执行错误")
        await whitelist_del(playername)
        await say_group_info(config["admin_qun"], "[Bot-白名单]玩家{player}已解绑".format(player=playername))
        await say_group(config["player_qun"], '解绑成功~')
    else:
        await say_group_warning(config["player_qun"], '该QQ名下未绑定该玩家，请检查ID是否正确')

# 黑名单-添加
async def banlist_add(playername, reason):
    db.ping(reconnect=True)
    ...
    if exist != 1:
        ...
        cursor.execute("INSERT INTO banlist(player_name, reason) VALUES ('{player}', '{reason}')".format(player=playername, reason=reason))
        statu = await sql_run()
        if statu != -1:
            await say_group_info(config["player_qun"], "玩家:{player} 原因:{reason} 已被移入黑名单,请各位引以为戒".format(player=playername, reason=reason))
            await say_group(config["admin_qun"], "[黑名单-添加]成功")
        else:
            await say_group(config["admin_qun"], "数据库执行错误")
    else:
        await say_group(config["admin_qun"], "该玩家已在黑名单内")

# 黑名单-删除
async def banlist_del(playername):
    db.ping(reconnect=True)
    ...
    if success == 0:
        ...
        if statu != -1:
            await say_group_info(config["player_qun"], "玩家:{player}已被移出黑名单".format(player=playername))
            await say_group(config["admin_qun"], "[黑名单-移除]成功")
        else:
            await say_group(config["admin_qun"], "数据库执行错误")
    else:
        await say_group(config["admin_qun"], "该玩家未被服务器封禁")
...
async def bot_add(bot_name, qq, purpose, open=0):
    if open == 1:
        cursor.execute("INSERT INTO usedbot(QQ,bot_name,isused,purpose) VALUES ('{QQ}','{bot_name}',true,'{purpose}')".format(QQ=qq, bot_name="bot_" + bot_name, purpose=purpose))
    else:
        cursor.execute("INSERT INTO usedbot(QQ,bot_name,isused,purpose) VALUES ('{QQ}','{bot_name}',false,'{purpose}')".format(QQ=qq, bot_name="bot_" + bot_name, purpose=purpose))
    success = await sql_run()
    if success != -1:
        if open == 0:
            await say_group(config["player_qun"], "[Bot-假人管理] 登记成功,假人未启用")
        else:
            await say_group(config["player_qun"], "[Bot-假人管理] 登记成功,假人已启用")
    else:
...
async def bot_del(botname, qq, admin=0):
    ...
    if success == 1:
        ...
        Q = output[0][0]
        if Q == qq or admin == 1:
            await sql_run("DELETE from usedbot where QQ = '{QQ}' and bot_name = '{bot}'".format(QQ=qq, bot="bot_" + botname))
...
async def get_bot(botname):
    success = cursor.execute("SELECT QQ,bot_name,isused,purpose,time from usedbot where bot_name = '{bot}'".format(bot="bot_" + botname))
    if success == 1:
        output = cursor.fetchall()
        QQ = output[0][0]
        bot_name = output[0][1]
        ...
        await say_group(config["player_qun"], "[Bot-假人管理]\n"
                             ...
                             "添加时间：{time}"
                             "正在使用: {isused}\n"
                             "用途: {purpose}".format(bot_name=bot_name, QQ=QQ, isused=use, purpose=purpose, time=time))
        del output
    else:
    ...
    if success == 0:
        ...
        for row in output:
            ...
            if isused == 1:
                ...
                use = "否"
            tempstr += "\n[{sum}]假人名称：{bot_name}\n是否使用：{use}\n用途：{purpose}\n创建时间：{time}\n".format(sum=int(id), bot_name=bot_name, use=use, purpose=purpose, time=time)
            # 打印结果
        del output
        await say_group(config["player_qun"], tempstr)
...
async def switch_bot(lenth, qq, botname, purpose=None):
    success = cursor.execute("SELECT QQ,bot_name,isused from usedbot where bot_name = '{bot}'".format(bot="bot_" + botname))
    if success:
        output = cursor.fetchall()
        dataQQ = output[0][0]
        bot_name = output[0][1]
        ...
        if lenth == 3:
            if qq == dataQQ:
                if isused == 1:
                    cursor.execute("UPDATE usedbot SET isused = false WHERE (`bot_name` = '{bot}')".format(bot=bot_name))
                    success = await sql_run()
                    if success != -1:
        ...
        # 更改用途
            ...
            if qq == dataQQ:
                cursor.execute("UPDATE usedbot SET purpose = '{purpose}' WHERE (`bot_name` = '{bot}')".format(bot=bot_name, purpose=purpose))
                success = await sql_run()
                if success != -1:
                    await server_run("player {bot} kill".format(bot=bot_name))
...
# FAQ
async def FAQ(qun, msg):
    for row in config["FAQ"]:
...
async def get_op(qq, playername):
    ...
    print(Inquire_1, Inquire_2, Inquire_3, Inquire_4)
    if Inquire_1 != 1 and Inquire_2 != 1:
        log.info("[审核]已收到玩家{player}的OP请求".format(player=playername))
...
async def refuse_one_op(id, reason=None):
    ...
    if Inquire_1 == 1:
        ...
        statu = await sql_run()
        if statu != -1:
            await say_group(config["admin_qun"], "玩家：{player}OP申请未通过\n退回原因：{info}".format(player=Player_Name, info=reason))
            await say_group_info(config["player_qun"], "玩家：{player}OP申请未审核通过\n原因：{info}".format(player=Player_Name, info=reason))
        else:
            await say_group(config["admin_qun"], "数据库执行错误，请检查")
    else:
        await say_group_warning(config["admin_qun"], "ID：{id}不存在，请重新查询".format(id=id))
